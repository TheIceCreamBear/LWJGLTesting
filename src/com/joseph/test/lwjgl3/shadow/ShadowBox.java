...
// another class from our man Karl, interesting, has to do with shadows and stuff
// gonna leave it like this for now, only modification was format and making it compile
...
public class ShadowBox {
	 ...
	 * 
	 * @param lightViewMatrix - basically the "view matrix" of the light. Can be
	 *            used to transform a point from world space into "light" space
	 *            (i.e. changes a point's coordinates from being in relation to
	 *            the world's axis to being in terms of the light's local axis).
	...
	protected void update() {
		...
		Vector3f forwardVector = new Vector3f(temp.x, temp.y, temp.z);
	...
	protected Vector3f getCenter() {
		...
		Matrix4f invertedLight = new Matrix4f(lightViewMatrix);
		invertedLight.invert();
		Vector4f result = invertedLight.transform(cen);
	 ...
	 * @param forwardVector - the direction that the camera is aiming, and thus
	...
	private Vector4f[] calculateFrustumVertices(Matrix4f rotation, Vector3f forwardVector, Vector3f centerNear, Vector3f centerFar) {
		Vector4f upTransform = rotation.transform(UP);
		Vector3f upVector = new Vector3f(upTransform.x, upTransform.y, upTransform.z);
	...
	private void calculateWidthsAndHeights() {
		farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(MainRenderer.FOV)));
		nearWidth = (float) (MainRenderer.NEAR_PLANE * Math.tan(Math.toRadians(MainRenderer.FOV)));
	...
	private float getAspectRatio() {
		return (float) GLFWHandler.SCREEN_WIDTH / (float) GLFWHandler.SCREEN_HEIGHT;
	}
